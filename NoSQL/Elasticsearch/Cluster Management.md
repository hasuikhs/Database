...
##### 루씬 텀 길이 제약
  - `keyword`나 `text` 타입 등 `string` 필드에 매누 긴 데이터가 들어오면 루씬 텀 길이 제약에 걸려 문서의 색인이 거부되는 경우가 생김
  - 특히, `keyword` 타입은 필드 내용을 통째로 하나의 텀으로 만들기에 발생하기 쉬움
  - 루씬의 최대 텀 길이는 32,766바이트
    - UTF-8 텍스트는 많은 바이트를 차지하기 때문에 신경 슬 필요가 있음
    - 매핑에 `ignore_above`를 지정하면 지정한 수치보다 긴 텀을 역색인에 넣지 않도록 함
    - 해당 값은 바이트 단위가 아닌 문자 단위
    - `text` 타입에는 지정 불가능

##### 대량 색인이 필요할 때
  - 서비스 출시를 위한 초기 데이터 세팅이나 마이그레이션, reindex를 통한 인덱스 재생성 등 대량 색인이 필요할 때가 있음
  - 이 경우에는 색인 도중 데이터를 조회할 필요 없음
    ...
    - refresh를 끄고 복제본 샤드 개수를 0으로 지정해서 복제본 생성을 중지
    - 작업이 끝나면 원래 설정으로 복구해야 함

## 6.4 샤드 운영 전략

- 인덱스의 샤드 개수(`number_of_shards`)는 한 번 지정하면 reindex 등의 특별한 작업을 수행하지 않는 한 변경 불가능
- 샤드 개수를 어떻게 지정하느냐에 따라 ES 클러스터 전체의 성능이 크게 달라짐

##### 샤드의 크기와 개수 조정
  - 클러스터에 샤드 숫자가 너무 많아지면 클러스터 성능이 눈에 띄게 떨어짐
  - 샤드 하나당 루씬 인덱스가 하나씩 더 뜨며 힙을 차지함
    - 주 샤드를 하나 더 띄울 때마다 복제본 샤드도 늘어나는 것을 고려하면 무작정 `number_of_shards`를 늘릴 수는 없음
    - 그렇다고 숫자를 적게 지정하면 샤드 하나당 크기가 커져 샤드 복구, 복제본 샤드 생성에 많은 시간이 소요되고 클러스터 안정성이 크게 떨어짐
  - 샤드 하나의 크기를 20~40GB 가 적절하다고 소개하지만, 실제 운영 상에서는 수 GB 수준에서 조정하는 것이 좋음

##### 모든 노드가 충분히 일을 하고 있는지
  - ES의 샤드는 분산처리를 위해 생긴 개념
  - 특정 인덱스의 주 샤드와 복제본 샤드가 모든 노드에 고르게 퍼지도록 설정하는 것이 중요
    - 노드 대수가 n대라면 `number_of_shards`를 n의 배수로 지정해 모든 노드가 작업을 고르게 분산받도록 설정하는 방법을 이용
    - 특히, 서비스 중요도가 높은 인덱스나 성능을 타이트하게 조정해야 한다면 신경써야 함

##### 미래에 데이터가 커질 것을 고려
  - 서비스 요건상 데이터의 성질이 변경되어 새로운 필드가 많이 추가되거나 데이터 크기 자체가 커질 수 있음
  - 이런 이슈를 고려해 데이터 사이즈를 어느 정도 고려해서 설정해야 함